...
redepositionDates$time <- redepositionDates$time - 60*60*24 # a day lag
...
# Turn into time objects
summonDates$time.x <- as.POSIXct(summonDates$time.x)
summonDates$time.y <- as.POSIXct(summonDates$time.y)
...
as.numeric(mean(summonDates[!duplicated(summonDates$node),]$time.x - 
                  summonDates[!duplicated(summonDates$node),]$time.y,na.rm=TRUE)) / (60*60*24) # mean = 0.64 day
as.numeric(range(summonDates[!duplicated(summonDates$node),]$time.x - 
                   summonDates[!duplicated(summonDates$node),]$time.y,na.rm=TRUE)) / (60*60*24) # between 0 and 5 days max
...
for(i in 1:nrow(summonDates)){
  if(is.na(summonDates$time.y[i])){
    summonDates$time.y[i] <- summonDates$time.x[i] - 60*60*24 # a day lag
  }
}
...
tortureDates$time <- as.POSIXct(tortureDates$time)
tortureDates$time <- tortureDates$time - 60*60*24 # a day-lag
